...
//
// Description:
// Assignment 4 project, see alternate point light positioning controls in parseKeys()
// 
//
//*****************************************************************************
using namespace std;
...
// Texture variables.
GLuint riddlerQuestionMarksID, alexID, blankID, brickID;
...
void init(void)
{
	...
	// Load second image.
	image = stbi_load("../Assets/img/riddler_question_marks.jpg", &width, &height, &bitDepth, 0);
	if (!image) { cout << "Unable to load file!" << endl; }
	glGenTextures(1, &riddlerQuestionMarksID);
	glBindTexture(GL_TEXTURE_2D, riddlerQuestionMarksID);
	// Note: image types with native transparency will need to be GL_RGBA instead of GL_RGB.
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	// glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	glTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_REPEAT);
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("GAME2012_A4_JarlettMark");
