...
void init(void)
{
	...
	// Load second image. Reusing image.
	image = stbi_load("alex.jpg", &width, &height, &bitDepth, 0);
	if (!image) { cout << "Unable to load file!" << endl; }
...
void display(void)
{
	...
	glBindVertexArray(vao);
		...
		transformObject(glm::vec3(2.0f, 2.0f, 2.0f), X_AXIS, 0.0f, glm::vec3(2.5f, 0.0f, -3.5f));
...
int main(int argc, char** argv)
{
	...
	glutCreateWindow("Week 10 - 03_CubeTexture.");
