...
void display(void)
{
	...
	glBindVertexArray(vao);
		...
		g_prism.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo);
		transformObject(glm::vec3(1.0f, 1.0f, 1.0f), X_AXIS, 0.0f, glm::vec3(0.0f, 0.0f, 0.0f));
		glDrawElements(GL_LINE_STRIP, g_prism.NumIndices(), GL_UNSIGNED_SHORT, 0);

		g_cone.ColorShape(1.0f, 0.5f, 0.0f);
		g_cone.BufferShape(&ibo, &points_vbo, &colors_vbo, &uv_vbo);
